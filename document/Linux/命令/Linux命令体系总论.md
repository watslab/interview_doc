...
| `type -a` | 列出所有匹配（含内建与外部） | 列出 `$PATH` 中所有同名可执行文件 | 列出别名与底层命令 |
...
带参数的选项：短选项紧贴参数 `wc -w file` 或 `cut -d: -f1`；长选项用 `=` 或空格 `--delimiter=: ` 或 `--delimiter :`。
...
> **顺序敏感**：`cmd > f 2>&1` 与 `cmd 2>&1 > f` 结果不同。前者将 stderr 合并到 stdout 后再一起写入文件；后者先将 stderr 合并到屏幕（当时的 stdout），再将 stdout 写入文件，stderr 仍输出到屏幕。
...
| `{a,b,c}` | 花括号展开（非文件匹配，生成组合） | `file{.txt,.log}` | file.txt file.log |

> 花括号展开不要求文件存在，它纯粹是字符串生成；而 `*`、`?`、`[]` 仅匹配实际存在的文件名，无匹配时按 Shell 选项决定行为（默认原样传递，`failglob` 报错）。
...
> **关键**：展开在第 4 步，重定向在第 5 步，命令查找在第 6 步——这意味着变量展开发生在命令查找之前，但重定向在查找之前已设置。引号能阻止展开：单引号阻止所有展开，双引号仅阻止单词拆分与路径名展开。
