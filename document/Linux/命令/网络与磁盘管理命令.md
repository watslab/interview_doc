...
PV 初始化时被划分为等大的 PE（Physical Extent，默认 4MiB），汇聚为 VG 的可用空间池。LV 从 VG 中分配空间时以 PE 为最小单位，因此 LV 大小必须是 PE 的整数倍。
...
COW 区存放原 LV 被修改前的旧数据块。COW 区满后，后续被修改的数据块无法再保存旧内容，快照读取这些块时只能得到新数据，冻结状态失败，因此快照失效。COW 区大小取决于快照存活期间原 LV 的写入量，通常设为原 LV 的 10%~20%。
