...
| SID | 会话 ID | 通常与终端关联，守护进程通过 `setsid()` 脱离控制终端 |
...
| Z | 僵尸进程 | EXIT_ZOMBIE | 已 exit() 但父进程尚未 wait() 回收，仅保留 task_struct |
...
- **守护进程（Daemon）**：脱离任何控制终端、在后台长期运行的系统服务。创建守护进程的典型步骤：`fork()` 后父进程退出 → 子进程 `setsid()` 成为会话首进程 → 再次 `fork()` 脱离会话首进程身份（防止重新获取终端）→ `chdir("/")` → `umask(0)` → 关闭并重定向标准 I/O。现代 Linux 中，systemd 直接管理大多数守护进程，无需手动 daemonize。
...
| 信号 | 编号 | 默认动作 | 说明 |
|------|------|----------|------|
| SIGHUP | 1 | Term | 控制终端挂断；常用于通知守护进程重载配置 |
| SIGINT | 2 | Term | 键盘中断，对应 Ctrl+C |
| SIGQUIT | 3 | Core | 键盘退出，对应 Ctrl+\，产生核心转储 |
| SIGABRT | 6 | Core | `abort()` 触发，通常由 assert 失败引发 |
| SIGKILL | 9 | Term | **强制终止，不能被捕获、阻塞或忽略** |
| SIGSEGV | 11 | Core | 非法内存访问（段错误） |
| SIGPIPE | 13 | Term | 向无读取端的管道写数据 |
| SIGALRM | 14 | Term | `alarm()` 定时器到期 |
| SIGTERM | 15 | Term | 请求正常终止（`kill` 默认信号），可被捕获以清理资源 |
| SIGCHLD | 17 | Ign | 子进程状态改变（退出 / 停止 / 继续）时通知父进程 |
| SIGCONT | 18 | Cont | 恢复被停止的进程 |
| SIGSTOP | 19 | Stop | **强制停止，不能被捕获、阻塞或忽略** |
| SIGTSTP | 20 | Stop | 终端停止信号，对应 Ctrl+Z，可被捕获 |
| SIGUSR1 / SIGUSR2 | 10 / 12 | Term | 用户自定义信号 |
...
### 5.4 Service 的 Type 类型

`[Service]` 段的 `Type=` 决定 systemd 如何判断服务启动完成：

| Type | 启动完成标志 | 适用场景 |
|------|-------------|----------|
| `simple`（默认） | `ExecStart` 启动的进程即为主进程，启动命令返回即视为就绪 | 前台运行的服务 |
| `forking` | `ExecStart` 进程 fork 出子进程后父进程退出，子进程为主进程 | 传统守护进程（如 Nginx、MySQL） |
| `oneshot` | `ExecStart` 进程执行完毕即视为完成 | 一次性脚本任务 |
| `notify` | 服务通过 `sd_notify()` 主动通知就绪 | 支持 systemd 通知协议的服务 |
| `dbus` | 服务在 D-Bus 上注册名称后视为就绪 | D-Bus 服务 |

### 5.5 journalctl — 日志查询

systemd 通过 journald 收集系统与服务日志（二进制格式，存储于 `/var/log/journal/`），使用 `journalctl` 统一查询：
...
> **buffer 与 cache 的区别**：
> - **buffer（缓冲区）**：用于暂存块设备（磁盘）读写数据，对应内核 `buffer cache`，加速磁盘写操作。
> - **cache（页缓存）**：用于缓存从文件读取的文件数据，对应内核 `page cache`，加速文件读操作。
> - 两者均可在内存压力时被内核回收，因此评估可用内存应以 `available` 而非 `free` 为准。
...
| 审计 | 不记录执行的命令 | 记录每条命令到日志（`/var/log/secure`） |
...
|----------|------|
| 1 | 用户名 |
| 2 | 密码哈希（`$id$salt$hash`，`$6$` 为 SHA-512；`!` 或 `*` 表示禁用密码登录） |
...
`dmesg` 打印内核环缓冲区（ring buffer）中的日志，记录硬件检测、驱动加载、错误等启动阶段信息：
...
# 查看正在使用 /var/log 目录的进程（排查"设备忙"无法卸载）
lsof +D /var/log
```
