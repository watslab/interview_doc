...
NDB Cluster 的 NDB 存储引擎是一种"内存优先"的存储引擎，支持两种表存储方式：

| 存储方式 | 说明 | 适用场景 |
|----------|------|----------|
| **内存表**（默认） | 表数据完全存储在内存中，访问速度极快 | 对延迟敏感的核心业务表 |
| **磁盘表** | 非索引列存储在磁盘，索引仍在内存中 | 数据量大、访问频率低的表 |

> **注意**：单个表只能选择一种存储方式，但同一集群中可以共存两种类型的表。

**内存表的持久化机制**：

由于内存数据断电会丢失，NDB 存储引擎通过以下机制保证数据持久性：

| 机制 | 说明 |
|------|------|
| **Redo Log** | 记录所有数据变更操作，写入磁盘 |
| **LCP（本地检查点）** | 定期将内存数据快照写入磁盘 |
| **GCP（全局检查点）** | 确保所有节点数据一致性 |
