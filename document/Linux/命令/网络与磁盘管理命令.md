...
| 统计项 | 含义 |
|--------|------|
| Total | 套接字总数（kernel 为内核分配数） |
| estab | ESTABLISHED 状态连接数 |
| closed | 已关闭但未回收的 TCP 套接字 |
| timewait | TIME_WAIT 状态连接数（含/后为定时器数） |
| orphaned | 孤儿套接字（已无对应文件描述符） |

### 3.2 netstat — 旧版连接查看工具
...
[iptables](https://www.netfilter.org/) 是 Linux 内核 Netfilter 框架的用户态配置工具。Netfilter 在内核网络协议栈的若干挂载点（hook）上拦截数据包，按规则链（chain）匹配处理。iptables 的核心结构为"表（table）→ 链（chain）→ 规则（rule）"。
...
| 表 | 优先级 | 作用 | 可用链 |
|----|--------|------|--------|
| `raw` | 最高 | 在连接跟踪前处理，可标记连接跳过 NAT | PREROUTING, OUTPUT |
| `mangle` | 高 | 修改数据包 TTL/TOS/DSCP 等 | 全部五链 |
| `nat` | 中 | 网络地址转换（DNAT/SNAT/MASQUERADE） | PREROUTING, OUTPUT, POSTROUTING |
| `filter` | 低 | 过滤放行 / 丢弃 | INPUT, FORWARD, OUTPUT |
...
同一链中表的执行顺序为 raw → mangle → nat → filter。规则匹配从上至下，命中即执行目标（ACCEPT/DROP/REJECT/MASQUERADE 等），不再继续匹配。
...
GPT（GUID Partition Table，[RFC 7646](https://www.rfc-editor.org/rfc/rfc7646) 相关标准为 UEFI 规范）支持超过 2TiB 磁盘、最多 128 个分区，是现代磁盘的默认分区表格式。
...
> **MBR vs GPT 选择**：磁盘超过 2TiB 必须使用 GPT（MBR 用 32 位 LBA 寻址，最大 2^32 × 512 = 2TiB）；需要超过 4 个主分区用 GPT；UEFI 系统引导盘必须 GPT，传统 BIOS 可用 MBR。新磁盘一律推荐 GPT，除非需要兼容旧系统。

## 十、文件系统管理

文件系统管理命令用于在分区上创建文件系统、检查修复、挂载与配置开机挂载。
...
> **ext4 与 xfs 修复差异**：ext4 使用 `fsck.ext4`（或 `e2fsck`）修复，支持离线修复；xfs 使用 `xfs_repair` 修复，同样要求文件系统已卸载。xfs 在线修复能力有限，严重损坏时可能需 `xfs_repair -L`（清空日志，有数据丢失风险）。两者均禁止在已挂载状态下运行修复工具。
...
```bash
# 验证 fstab 配置（不实际挂载，仅检查语法，建议修改后必执行）
sudo mount -a
# 查询设备 UUID
sudo blkid /dev/sdb1
```
...
LVM 快照（snapshot）是 LV 在某时刻的只读视图，基于写时复制（Copy-On-Write）：快照创建时不复制数据，仅在原 LV 数据块被修改时，将旧数据块拷贝到快照区。快照常用于一致性备份。

```bash
# 创建快照（-L 指定快照区大小，约为原 LV 变化量的预估）
sudo lvcreate -L 5G -s -n lv_data_snap /dev/vg_data/lv_data

# 挂载快照（用于备份）
sudo mount /dev/vg_data/lv_data_snap /mnt/snap

# 备份完成后删除快照（快照区不可占满，否则快照失效）
sudo umount /mnt/snap
sudo lvremove /dev/vg_data/lv_data_snap
```

> **快照区大小**：快照区（COW 区）存放原 LV 被修改的旧数据块。快照区满则快照失效（变为不可用）。快照区大小取决于快照存活期间原 LV 的写入量，通常设为原 LV 的 10%–20%，备份完成后立即删除。
