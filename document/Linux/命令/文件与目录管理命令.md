...
touch -c file.txt            # 更新 atime 与 mtime，若文件不存在则不创建
touch -a file.txt            # 仅更新 atime（文件不存在时仍会创建）
touch -m file.txt            # 仅更新 mtime（文件不存在时仍会创建）
touch -ac file.txt           # 仅更新 atime，且文件不存在时不创建
...
> 数字法无法仅对某一类用户（所有者/所属组/其他人）做增量调整（如仅 `+x`），必须同时给出三组完整值；符号法适合增量调整，数字法适合一次性设定全部权限。
...
除基本 rwx 权限外，Linux 提供三种特殊权限位，用于实现特定安全场景。三种特殊权限位是三个独立位，可以同时设置多项，数字前缀为各值之和：SUID=4，SGID=2，Sticky=1。
...
chmod 6755 /usr/bin/myapp       # 同时设置 SUID + SGID（4+2=6）
chmod 7755 /shared/project      # 同时设置 SUID + SGID + Sticky（4+2+1=7）
```
...
gzip、bzip2、xz 既可被 `tar` 通过 `-z`/`-j`/`-J` 选项调用，也可单独使用。单独使用时每次压缩一个文件（`gzip -r` 可递归压缩目录下所有文件，但每个文件独立生成 .gz，而非打包为单个归档），压缩后默认删除原文件（`-k` 保留原文件）。zip 保留原文件。
...
实际运维与开发中，常通过管道串联命令并配合选项调整行为，来组合完成复杂任务。以下列出十个高频实用组合。
